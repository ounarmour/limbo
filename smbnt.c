...
void showUsage()
{
  writeVerbose(VB_NONE, "--> we can make changes, hazaa <-- \n\n");
...
void lrn_toString_psSessionData(int iLevel, _SMBNT_DATA *psSessionData)
{
	writeError(iLevel, "(lrn) dump psSessionData");
	writeError(iLevel, "(lrn) psSessionData->challenge         %s", psSessionData->challenge);
	writeError(iLevel, "(lrn) psSessionData->workgroup         %s", psSessionData->workgroup);
	writeError(iLevel, "(lrn) psSessionData->workgroup_other   %s", psSessionData->workgroup_other);
	writeError(iLevel, "(lrn) psSessionData->machine_name      %s", psSessionData->machine_name);
	writeError(iLevel, "(lrn) psSessionData->security_mode     %d", psSessionData->security_mode);
	writeError(iLevel, "(lrn) psSessionData->authLevel         %d", psSessionData->authLevel);
	writeError(iLevel, "(lrn) psSessionData->hashFlag          %d", psSessionData->hashFlag);
	writeError(iLevel, "(lrn) psSessionData->accntFlag         %d", psSessionData->accntFlag);
	writeError(iLevel, "(lrn) psSessionData->protoFlag         %d", psSessionData->protoFlag);
}
...
*/
void lrn_toString_psLogin(int iLevel, sLogin *psLogin)
{
	writeError(iLevel, "(lrn) dump psLogin");
	writeError(iLevel, "(lrn) psLogin->psServer      0x%010x", psLogin->psServer);
	writeError(iLevel, "(lrn) psLogin->psUser        0x%010x", psLogin->psUser);
	writeError(iLevel, "(lrn) psLogin->iResult       %d", psLogin->iResult);
	writeError(iLevel, "(lrn) psLogin->pErrorMsg     %s", psLogin->pErrorMsg);
	writeError(iLevel, "(lrn) psLogin->iId           %d", psLogin->iId);
	writeError(iLevel, "(lrn) psLogin->iLoginsDone   %d", psLogin->iLoginsDone);
}
...
int go(sLogin* logins, int argc, char *argv[])
{
  ...
  {
    writeError(ERR_DEBUG_MODULE, "OMG the %s module has been called!!", MODULE_NAME);
    ...
    lrn_toString_psSessionData(ERR_DEBUG_MODULE, psSessionData);	// *LRN
...
int initModule(sLogin* psLogin, _SMBNT_DATA *_psSessionData)
{
  ...
  lrn_toString_psLogin(ERR_DEBUG_MODULE, psLogin);	// *LRN
  ...
  lrn_toString_psSessionData(ERR_DEBUG_MODULE, _psSessionData);	// *LRN
